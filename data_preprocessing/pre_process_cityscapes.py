...
def make_dataset(data_dir, image_flag=True):
    train_list = []
    val_list = []
    train_folder_list = glob.glob(os.path.join(data_dir, '*'))
    val_folder_list = glob.glob(os.path.join(data_dir, '*'))
    ...
    if image_flag: 
        ...
        for folder_path in train_folder_list:
            for file_path in glob.glob(os.path.join(folder_path, '*xxxx_.png'))
                train_list.append(file_path)       
...
def generate_image_dataset(train_data_dir_out, val_data_dir_out,
    ...
    for file_name in tqdm(image_train_list, total= len(image_train_list), 
                           desc='Start train image proccessing'):
        folder_name = file_name.split(os.path.sep)[-2]
        temp_folder = os.path.join(train_data_dir_out, folder_name)
        file_name = file_name.split(os.path.sep)[-1].split('.')[0] + '_img.jpg'
        temp_file_name = os.path.join(temp_folder, file_name)

        if not os.path.exists(temp_folder):
            os.mkdir(temp_folder)
        
        pool.apply_async(save_images, (file_name, temp_file_name))


    for file_name in tqdm(image_val_list, total= len(image_val_list), 
                           desc='Start val image proccessing'):
        folder_name = file_name.split(os.path.sep)[-2]
        temp_folder = os.path.join(val_data_dir_out, folder_name)
        file_name = file_name.split(os.path.sep)[-1].split('.')[0] + '_img.jpg'
        temp_file_name = os.path.join(temp_folder, file_name)
...
def generate_label_dataset(train_data_dir_out, val_data_dir_out,
    ...
    for file_name in tqdm(lable_train_list, total= len(lable_train_list), 
                           desc='Start train label proccessing'):
        folder_name = file_name.split(os.path.sep)[-2]
        file_name = file_name.split(os.path.sep)[-1].split('.')[0] + '_label.png'
        temp_file_name = os.path.join(train_data_dir_out, folder_name, file_name)
        shutil.copyfile(file_name, temp_file_name)

    for file_name in tqdm(label_val_list, total= len(label_val_list), 
                           desc='Start val label proccessing'):
        folder_name = file_name.split(os.path.sep)[-2]
        file_name = file_name.split(os.path.sep)[-1].split('.')[0] + '_label.png'
        temp_file_name = os.path.join(val_data_dir_out, folder_name, file_name)
        shutil.copyfile(file_name, temp_file_name)
...
def main():
    ...
    image_data_dir_in = os.path.join(data_dir_in, 'xxxx_.')
    label_data_dir_in = os.path.join(data_dir_in, 'xxxx_.')
    ...
    generate_image_dataset(train_data_dir_out, val_data_dir_out,
                           image_train_list, image_val_list)
    generate_label_datasent(train_data_dir_out, val_data_dir_out,
                            lable_train_list, label_val_list)
